...
// per-row pixel counts and column extents for each class
const classes = ['beige', 'orange', 'muted', 'other'];
const rows = {};
for (const c of classes) rows[c] = [];

for (let y = 0; y < height; y++) {
  const counts = { beige: 0, orange: 0, muted: 0, other: 0 };
  const min = { beige: width, orange: width, muted: width, other: width };
  const max = { beige: -1, orange: -1, muted: -1, other: -1 };
